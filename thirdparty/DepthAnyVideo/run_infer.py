...
if "__main__" == __name__:
    ...
    is_folder = os.path.isdir(cfg.data_path)
    ...
    if is_video:
        ...
        file_name = os.path.basename(cfg.data_path)
        image = list(map(lambda x: img_utils.read_image(os.path.join(cfg.data_path, x))[0], sorted(os.listdir(cfg.data_path))))
    ...
    if is_video:
        ...
        for i in range(num_frames):
            img_utils.write_image(
                os.path.join(cfg.output_dir, f"{file_name}_depth_{i:04d}.png"),
                disparity_colored[i],
            )
        from PIL import Image
        pil_images = [Image.fromarray(disparity.astype(np.uint8)) for disparity in disparity_colored]
        pil_images[0].save(os.path.join(cfg.output_dir, 'depth.gif'), save_all=True, append_images=pil_images[1:], duration=100, loop=0)
    else:
        img_utils.write_image(
