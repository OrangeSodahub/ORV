...
from training.args import parse_args  # isort:skip
from training.text_encoder import compute_prompt_embeddings  # isort:skip
from training.utils import get_gradient_norm, get_optimizer, prepare_rotary_positional_embeddings, print_memory, reset_memory, flatten_dict, CONSOLE  # isort:skip
from dataset.dataset import BucketSampler, RobotDataset, MultiViewRobotDataset, CollateFunctionControl  # isort:skip
...
# def log_test(
#     accelerator: Accelerator,
#     pipe: Union[CogVideoXPipeline, CogVideoXImageToVideoPipelineTraj, CogVideoXImageToVideoPipelineTrajControl],
#     vae: Union[AutoencoderKLCogVideoX, None],
#     dataset: Union[RobotDataset, None],
#     args: Dict[str, Any],
#     pipeline_args: Dict[str, Any],
#     epoch,
...
# ):
#     logger.info(
#         f"Running test... \n Generating {config.num_validation_videos} videos with prompt: {pipeline_args['prompt']}."
#     )

#     tracking_map_path = pipeline_config.pop("tracking_map_path", None)

#     try:
#         tracking_maps = pipeline_config.pop("tracking_maps", None)
#     except:
#         tracking_maps = None
    
#     if tracking_map_path and config.load_tensors is False:
...
#     if config.num_validation_videos > 2:
#         config.num_validation_videos = 2  # 限制验证视频数量

#     videos = []
#     for _ in range(config.num_validation_videos):
#         with torch.no_grad():
...
#                     .replace("/", "_")
#                 )
#                 filename = os.path.join(config.output_dir, f"{phase_name}_ep{epoch}_{i}th_{prompt}.mp4")
...
def main(config):
    ...
    dataset_kwargs = {
        ...
        # 'height_buckets': config.height_buckets,
        # 'width_buckets': config.width_buckets,
