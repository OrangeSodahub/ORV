...
from dataset.dataset import RobotDataset, MultiViewRobotDataset  # isort:skip
...
def get_args() -> Dict[str, Any]:
    ...
    # parser.add_argument("--max_num_frames", type=int, default=100, help="Maximum number of frames in output video.")
    parser.add_argument(
    ...
    for step, batch in enumerate(iterator):
        ...
        try:
            ...
            if is_process('depthGT_latents'):
                ...
                if args.use_slicing:
                    encoded_slices = [vae._encode(depth_slice) for depth_slice in depthsGT.split(1)]
                    depthGT_latents = torch.cat(encoded_slices)
                else:
                    depthGT_latents = vae._encode(depthsGT)
            ...
            if is_process('labelGT_latents'):
                ...
                if args.use_slicing:
                    encoded_slices = [vae._encode(label_slice) for label_slice in labelsGT.split(1)]
                    labelGT_latents = torch.cat(encoded_slices)
                else:
                    labelGT_latents = vae._encode(labelsGT)
