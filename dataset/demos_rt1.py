...
def generate_occupancy():
    ...
    try:
        ...
        masks2d = (points2d[:, 0] >= 0) & (points2d[:, 0] < 320) & (points2d[:, 1] >= 0) & (points2d[:, 1] < 256)
...
def generate_labels():
    ...
    if False:
        ...
        text = 'robot gripper, metal table, white plate, robot arm, orange ball.'
        ...
        for i, object in enumerate(OBJECTS):
            if object in traj_labels and object not in [OBJECTS[i] for i in valid_indices]:
...
def extract_mp4_to_images():
    ...
    for mp4_path in tqdm(mp4_paths):
        ...
        save_folder = Path('outputs2/demos/') / f'{video_path.parent.name}_{video_path.stem}'
...
if __name__ == '__main__':
    ...
    data_root = '/share/project/cwm/xiuyu.yang/work/dev6/data/'
